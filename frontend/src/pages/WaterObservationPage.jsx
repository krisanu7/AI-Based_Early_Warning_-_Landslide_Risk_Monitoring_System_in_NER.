...
export const WaterObservationPage = () => {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-900 to-teal-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block font-bold text-slate-700 mb-1">Water Source Type</label>
                <select
            ...
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
              <div className="font-bold text-slate-800 text-[11px] uppercase tracking-wider">Field Test Kit Measurements</div>
              
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Turbidity (NTU)</label>
                  <input
                    type="number"
                    step="0.1"
                    min="0"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2 rounded-xl border border-slate-200 font-bold text-slate-900"
                  />
                  <span className="text-[10px] text-slate-400">Normal: 6.5 – 8.5</span>
                </div>

                <div>
                  <label className="block font-bold text-slate-700 mb-1">Residual Chlorine</label>
                  <input
                    type="number"
                    step="0.1"
                    min="0"
                    ...
                    className="w-full p-2 rounded-xl border border-slate-200 font-bold text-slate-900"
                  />
                  <span className="text-[10px] text-slate-400">Target: &gt; 0.2 ppm</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <label className={`p-3 rounded-xl border cursor-pointer flex items-center gap-2.5 transition-all ${
                  formData.coliform_presence ? 'bg-rose-50 border-rose-300 text-rose-900 font-bold' : 'bg-white border-slate-200 text-slate-600'
                }`}>
                  <input
                    type="checkbox"
                ...
                <label className={`p-3 rounded-xl border cursor-pointer flex items-center gap-2.5 transition-all ${
                  formData.is_flood_affected ? 'bg-amber-50 border-amber-300 text-amber-900 font-bold' : 'bg-white border-slate-200 text-slate-600'
                }`}>
                  <input
                    type="checkbox"
            ...
            <div>
              <label className="block font-bold text-slate-700 mb-1">Field Analyst Notes</label>
              <textarea
                rows={2}
        ...
        <div className="lg:col-span-5 space-y-6">
          {result && (
            <div className="p-5 rounded-3xl bg-white border border-blue-200 shadow-sm space-y-2 animate-in fade-in">
              <div className="flex items-center gap-2 font-bold text-blue-950 text-sm">
                <Sparkles className="w-4 h-4 text-blue-600" />
                <span>Assessment Generated</span>
              </div>
              <div className="text-xs font-semibold text-slate-700">
                Water Safety Classification: <strong className="text-blue-700">{result.rating}</strong>
              </div>
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <h3 className="font-bold text-slate-900 text-sm">Recent Water Quality Tests</h3>
              <span className="text-[11px] text-slate-400">{observations.length} logs</span>
            </div>

            <div className="space-y-3 max-h-[420px] overflow-y-auto">
              {observations.map((obs, idx) => (
                <div key={idx} className="p-3.5 rounded-2xl border border-slate-100 bg-slate-50/60 text-xs space-y-1.5">
                  <div className="flex items-start justify-between">
                    <span className="font-bold text-slate-900">{obs.village} ({obs.water_source})</span>
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-slate-600">
                    ...
                    <span>•</span>
                    <span className={obs.coliform_presence ? 'text-rose-600 font-bold' : 'text-emerald-700'}>
                      Coliform: {obs.coliform_presence ? 'POSITIVE' : 'Negative'}
                    </span>
                  </div>
                  {obs.notes && <p className="text-[10px] text-slate-500 italic truncate">{obs.notes}</p>}
                </div>
              ))}
            </div>
          </div>
