    ...
    <header className="sticky top-0 z-40 bg-white/85 dark:bg-slate-900/85 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <Link to="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-amber-600 via-rose-600 to-indigo-600 flex items-center justify-center text-white shadow-md shadow-rose-600/20 group-hover:scale-105 transition-transform">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {/* User Account / Login / Register / Logout Dropdown */}
          <div className="relative">
            {user ? (
              <>
                <button
                  onClick={() => { setUserMenuOpen(!userMenuOpen); setRoleMenuOpen(false); setLangMenuOpen(false); }}
                  className="px-2.5 sm:px-3 py-1.5 rounded-xl border border-rose-500/30 bg-rose-500/10 hover:bg-rose-500/20 text-rose-700 dark:text-rose-300 text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm"
                >
                  <User className="w-4 h-4 text-rose-500" />
                  <span className="hidden sm:inline font-black truncate max-w-[100px]">{user.name.split(' ')[0]}</span>
              ...
              <div className="flex items-center gap-1.5">
                <Link
                  to="/login"
                  className="px-3 py-1.5 rounded-xl border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-white text-xs font-bold flex items-center gap-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
                >
                ...
                <Link
                  to="/register"
                  className="hidden sm:flex px-3 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold items-center gap-1.5 shadow-md shadow-rose-600/20 transition-all"
                >
